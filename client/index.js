...
var trialApplications = mysqlTable("trial_applications", {
  id: int("id").autoincrement().primaryKey(),
...
async function createTrialApplication(application) {
  const db = await getDb();
  if (!db) {
...
async function getAllTrialApplications() {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
async function getTrialApplicationById(id) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
async function updateTrialApplicationStatus(id, status, notes) {
  const db = await getDb();
  if (!db) {
    throw new Error("Database not available");
  }
...
var appRouter = router({
  ...
  trialApplications: router({
    ...
    submit: publicProcedure.input(
      z2.object({
        hospitalName: z2.string().min(1, "\u533B\u9662\u540D\u79F0\u4E0D\u80FD\u4E3A\u7A7A"),
        contactName: z2.string().min(1, "\u8054\u7CFB\u4EBA\u59D3\u540D\u4E0D\u80FD\u4E3A\u7A7A"),
        contactPhone: z2.string().min(1, "\u8054\u7CFB\u7535\u8BDD\u4E0D\u80FD\u4E3A\u7A7A"),
        contactEmail: z2.string().email("\u90AE\u7BB1\u683C\u5F0F\u4E0D\u6B63\u786E").optional().or(z2.literal("")),
        department: z2.string().optional(),
        position: z2.string().optional(),
        productInterest: z2.enum(["qc", "generation", "both"]),
        hospitalScale: z2.enum(["small", "medium", "large", "extra_large"]).optional(),
...
\u4EA7\u54C1: ${input.productInterest === "qc" ? "\u75C5\u5386\u8D28\u63A7" : input.productInterest === "generation" ? "\u75C5\u5386\u751F\u6210" : "\u4E24\u8005\u90FD\u611F\u5174\u8DA3"}`
    ...
    list: protectedProcedure.query(async ({ ctx }) => {
      ...
      return await getAllTrialApplications();
    }),
    ...
    getById: protectedProcedure.input(z2.object({ id: z2.number() })).query(async ({ input, ctx }) => {
      ...
      return await getTrialApplicationById(input.id);
    }),
    // Protected: Update application status (admin only)
    updateStatus: protectedProcedure.input(
      z2.object({
        id: z2.number(),
        status: z2.enum(["pending", "contacted", "demo_scheduled", "converted", "rejected"]),
      ...
      await updateTrialApplicationStatus(input.id, input.status, input.notes);
      return { success: true };
    })
  })
